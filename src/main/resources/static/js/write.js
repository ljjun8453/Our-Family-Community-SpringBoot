...
(function () {
    ...
    function getCsrf() {
        ...
        if (xsrf) return {header: 'X-XSRF-TOKEN', token: decodeURIComponent(xsrf.split('=')[1])};
    ...
    async function uploadToServer(file) {
        ...
        if (!res.ok) throw new Error('업로드 실패: ' + (await res.text().catch(() => '')));
        return res.json(); // {url}
    }

    // 에디터 초기화
    const hidden = document.getElementById('contentHidden');   // hidden input 잡기
    ...
    // 제출 직전: hidden 값에 에디터 내용 저장
    window.__write_onSubmit = function () {
        if (hidden) {
            hidden.value = editor.getHTML();   // 덮어쓰기
        }
        console.log('content length:', hidden?.value?.length || 0);
        return true;
    };
})();
